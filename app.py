...
# 2. МЕСТО ПОД ПРЕВЬЮ И РАЗРЕШЕНИЕ (заполнится позже)
...
# Логика отрисовки в зарезервированные места
if w_mm > 0 and h_mm > 0 and pitch > 0:
    ...
    if logo_img and bg_files:
        ...
        if preview:
            ...
            # Рисуем разрешение ВЕРХУ
            res_area.markdown(f"<div class='res-box'>Разрешение: {tw} × {th} px</div>", unsafe_allow_html=True)

    # 4. КНОПКА (внизу)
    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("Обработать все изображения"):
        if not logo_img or not bg_files:
            st.error("Ошибка: Проверьте наличие логотипа и папки с фото.")
        else:
