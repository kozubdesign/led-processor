...
# Упрощенный CSS (только центрирование кнопки и стиль плашки)
st.markdown("""
    <style>
    .block-container { max-width: 800px !important; margin: 0 auto !important; }
    
    /* Центрируем кнопку и делаем её зеленой */
...
# 1. ШАПКА (СТРОГО ПЕРВАЯ)
st.header("Создать контент для LED-экрана")

# 2. ПУСТЫЕ ОБЛАСТИ (заполнятся позже, но место забронировано вверху)
...
if w_mm > 0 and h_mm > 0 and pitch > 0:
    ...
    if logo_img and bg_files:
        ...
        if preview:
            ...
            # Рендерим превью в placeholder №1
            ...
            # Рендерим разрешение в placeholder №2
